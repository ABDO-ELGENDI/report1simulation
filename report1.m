...
%%
%circle
x4=-2:0.1:2;%circle equation with zero origin and radius=2 x^2+y^2=r^2
z41=sqrt(4-power(x4,2));
z42=-sqrt(4-power(x4,2));
subplot(5,5,13);
plot(x4,z41,'m','LineWidth',2);%positive part
hold on;
plot(x4,z42,'m','LineWidth',2);%negative part
axis([-2 2 -2 2])%xstart xed ystart yend
%%
%triangular number
x5=1:1:10;
z5=(x5.^2+x5)/2;%[(n*(n+1))/2]=[(n^2+n)/2] the first one will cause error because of matrix multiplication rules
...
%tripuls wave
x9=-0.5:0.01:0.5;
z9=triangularPulse(2*x9);%tripulse is betwen -1 and 1 mul x9 by two two shrink the shape to be between -0.5 and 0.5
subplot(5,5,24);
...
axis([0 15 -1 1])
%%
set(gcf, 'Position',  [100, 50, 1100, 600])%for window position and size